...
func NewConfig(creds credentials.TransportCredentials, opts ...gorums.ManagerOption) *Config {
	...
	cfg := &Config{
		...
		optsPtr:               &opts,
		proposeCancel:         func() {},
		proposebatchmulCancel: func() {},
		timeoutCancel:         func() {},
		mzproposeCancel:       func() {},
	}
	return cfg
}
...
func (cfg *Config) ProposeBatchMultiCast(batch *consensus.Batch) {
	if cfg.cfg == nil {
		return
	}
	var ctx context.Context
	// cfg.proposebatchmulCancel()
	ctx, cfg.proposebatchmulCancel = context.WithCancel(context.Background())
...
//	//p := hotstuffpb.BatchToProto(batch)
//	//cfg.cfg.ProposeBatch_multicast(ctx, p, gorums.WithNoSendWaiting())
//}
...
func (cfg *Config) MzPropose(proposal consensus.MzProposeMsg) {
	if cfg.cfg == nil {
		return
	}
	var ctx context.Context
	cfg.mzproposeCancel()
	ctx, cfg.mzproposeCancel = context.WithCancel(context.Background())
	p := hotstuffpb.MzProposalToProto(proposal)
	cfg.cfg.MzPropose(ctx, p, gorums.WithNoSendWaiting())
}
