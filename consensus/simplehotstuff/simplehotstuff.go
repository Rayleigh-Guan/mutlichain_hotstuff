...
func (hs *SimpleHotStuff) CommitRule(block *consensus.Block) *consensus.Block {
	...
	if !ok {
		fmt.Println("Node Id:", hs.mods.ID(), " commit rule errors because can not find new block's parent --new block hash:", block.Hash().String(), " --parent block hash", block.QuorumCert().BlockHash().String())
		return nil
	}
	...
	if ok && gp.View() > hs.locked.View() {
		...
		fmt.Println("Node Id:", hs.mods.ID(), " commit rule errors because can not find new block's grandparent --new block hash:", block.Hash().String(), " --grandparent block hash", p.QuorumCert().BlockHash())
		return nil
	}
	//fmt.Println(gp.QuorumCert().BlockHash().String())
	...
	fmt.Println("Node Id:", hs.mods.ID(), " commit rule errors because can not find new block's ggrandparent --new block hash:", block.Hash().String(), " --ggrandparent block hash", gp.QuorumCert().BlockHash())
	//if !ok {
	//	fmt.Println("Node Id:", hs.mods.ID(), " commit rule errors because can not find new block's great-grandparent --new block hash:",
	//		block.Hash().String(), " --great-grandparent block hash", gp.QuorumCert().BlockHash())
	//} else {
	//	fmt.Println("Node Id:", hs.mods.ID(), " commit rule errors because view erroe --new block hash:",
	//		block.Hash().String(), " --great-grandparent block hash", gp.QuorumCert().BlockHash(), " --ggp.View:", ggp.View(), " --p.View", p.View())
	//}
