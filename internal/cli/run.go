...
func init() {
	...
	runCmd.Flags().Int("clients", 4, "number of clients to run")
	runCmd.Flags().Int("batch-size", 50, "number of commands to batch together in each block")
	runCmd.Flags().Int("payload-size", 512, "size in bytes of the command payload")
	runCmd.Flags().Int("max-concurrent", 3000, "maximum number of conccurrent commands per client")
	runCmd.Flags().Duration("client-timeout", 20000*time.Millisecond, "Client timeout.")
	runCmd.Flags().Duration("duration", 20000*time.Second, "duration of the experiment")
	runCmd.Flags().Duration("connect-timeout", 50*time.Second, "duration of the initial connection timeout")
	...
	runCmd.Flags().String("consensus", "simplehotstuff", "name of the consensus implementation")
