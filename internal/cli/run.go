...
func init() {
	...
	runCmd.Flags().Int("batch-size",800, "number of commands to batch together in each block")
	runCmd.Flags().Int("payload-size", 512, "size in bytes of the command payload")
	runCmd.Flags().Int("max-concurrent",600, "maximum number of conccurrent commands per client")
	runCmd.Flags().Duration("client-timeout", 200000*time.Millisecond, "Client timeout.")
	runCmd.Flags().Duration("duration", 10000*time.Second, "duration of the experiment")
	runCmd.Flags().Duration("connect-timeout", 5*time.Second, "duration of the initial connection timeout")
	runCmd.Flags().Duration("view-timeout", 10000*time.Millisecond, "duration of the first view")
