...
func init() {
	...
	runCmd.Flags().Int("batch-size", 4, "number of commands to batch together in each block")
	runCmd.Flags().Int("payload-size", 512, "size in bytes of the command payload")
	runCmd.Flags().Int("max-concurrent", 20, "maximum number of conccurrent commands per client")
	runCmd.Flags().Duration("client-timeout", 20000*time.Millisecond, "Client timeout.")
	runCmd.Flags().Duration("duration", 20*time.Second, "duration of the experiment")
