...
func MzProposalToProto(proposal consensus.MzProposeMsg) *MzProposal {
	p := &MzProposal{
		Block: MzBlockToProto(proposal.Block),
	}
	if proposal.AggregateQC != nil {
		p.AggQC = AggregateQCToProto(*proposal.AggregateQC)
	}
...
// ProposalFromProto converts a protobuf message to a ProposeMsg.
func MzProposalFromProto(p *MzProposal) (proposal consensus.MzProposeMsg) {
	proposal.Block = MzBlockFromProto(p.GetBlock())
	if p.GetAggQC() != nil {
		aggQC := AggregateQCFromProto(p.GetAggQC())
		proposal.AggregateQC = &aggQC
	}
...
// BlockToProto converts a consensus.MzBlock to a hotstuffpb.MzBlock.
func MzBlockToProto(block *consensus.MzBlock) *MzBlock {
	parentHash := block.Parent()
	blockHash := block.Hash()
	return &MzBlock{
		Parent:    parentHash[:],
		BatchList: []byte(block.BatchList()),
		QC:        QuorumCertToProto(block.QuorumCert()),
		View:      uint64(block.View()),
		CmdUnSync: []byte(block.Cmdunsync()),
		Proposer:  uint32(block.Proposer()),
		Hash:      blockHash[:],
	}
}
...
// BlockFromProto converts a hotstuffpb.Block to a consensus.Block.
func MzBlockFromProto(block *MzBlock) *consensus.MzBlock {
	var p consensus.Hash
	copy(p[:], block.GetParent())
	var h consensus.Hash
	copy(h[:], block.GetHash())
	return consensus.NewMzBlock(
		p,
		QuorumCertFromProto(block.GetQC()),
		consensus.BatchList(block.GetBatchList()),
		consensus.Command(block.GetCmdUnSync()),
		consensus.View(block.GetView()),
		hotstuff.ID(block.GetProposer()),
		h,
	)
...
func BatchFromProto(batch *Batch) (a consensus.Batch) {
