...
// ProposeBatchMultiCast is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) ProposeBatchMultiCast(ctx context.Context, in *Batch, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.ProposeBatchMultiCast",
	}
...
// MzPropose is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) MzPropose(ctx context.Context, in *MzProposal, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.MzPropose",
	}
...
func RegisterHotstuffServer(srv *gorums.Server, impl Hotstuff) {
	...
	srv.RegisterHandler("hotstuffpb.Hotstuff.ProposeBatchMultiCast", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*Batch)
		defer ctx.Release()
		impl.ProposeBatchMultiCast(ctx, req)
	})
	srv.RegisterHandler("hotstuffpb.Hotstuff.MzPropose", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*MzProposal)
		defer ctx.Release()
