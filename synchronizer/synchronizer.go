...
func (s *Synchronizer) Start(ctx context.Context) {
	...
	if s.currentView == 1 && s.mods.LeaderRotation().GetLeader(s.currentView) == s.mods.ID() {
		...
		s.mods.Consensus().MzPropose(s.SyncInfo())
	}
}
...
func (s *Synchronizer) AdvanceView(syncInfo consensus.SyncInfo) {
	...
	if leader == s.mods.ID() {
		...
		s.mods.Consensus().MzPropose(syncInfo)
...
func (s *Synchronizer) UpdateHighQC(qc consensus.QuorumCert) {
	...
	//fmt.Println("qc block:", qc.BlockHash().String(), " current view:", s.currentView)
	s.updateHighQC(qc)
}
...
func (s *Synchronizer) updateHighQC(qc consensus.QuorumCert) {
	...
	if !ok {
		s.mods.Logger().Info("Block from the old highQC missing from chain")
	}

	if newBlock.View() > oldBlock.View() {
		s.highQC = qc
		s.leafBlock = newBlock
		s.mods.Logger().Debug("HighQC updated", newBlock.Hash().String())
	}
